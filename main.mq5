...
bool Is1AMNewYork(datetime& time)
{
   ...
   int offset = isDstActive(time) ? UTCOffsetDst + UTCServerDst : UTCOffsetNonDst + UTCServerNonDst;  // Adjust for New York time (or user-defined UTC offset)
...
void CalculateHighLow(datetime& time) {
   ...
   int startShift = iBarShift(Symbol(), PERIOD_H1, time - (timeframe * 3600)); 
   
   // Get the highest high and lowest low from 9 PM to 1 AM
   data.high = iHigh(Symbol(),timeframe, startShift);
   data.low =  iLow(Symbol(), timeframe, startShift);

   for(int i = 0; i < timeframe; i++) {  // Loop through the 4 hours from 9 PM to 1 AM
